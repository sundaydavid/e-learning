<form id="signin_student" class="form-signin" method="post">
	...
	<input type="password" class="input-block-level" id="cpassword" name="cpassword" placeholder="Re-type Password"
		required>
	<button id="signin" name="login" class="btn btn-info" type="submit"><i class="icon-check icon-large"></i> Sign
		in</button>
</form>
			...
			if (password == cpassword) {
				...
				$.ajax({
					...
					success: function (html) {
						if (html == 'true') {
							$.jGrowl("Welcome to E-Learning System", { header: 'Sign up Success' });
							var delay = 2000;
